...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			`[data-stc-ghost]{position:absolute!important;width:1px!important;height:1px!important;overflow:hidden!important;` +
			...
			`[data-stc-ghost] *{color:transparent!important;background:transparent!important}` +
			`[data-stc-ghost]::before,[data-stc-ghost]::after{content:none!important}`,
		].join("");
		...
		let layer = null;
		let chip = null, panel = null, expanded = false, hoverOpen = false, hoverCloseTimer = 0, clearArmed = 0, clearTimer = 0;
		...
		// 把批注写进输入框：DSH 据此认为“有内容”（发送按钮可用），
		// 随后给这些段落打上隐藏标记，视觉上只留“N 条注释”小条。
		function insertIntoComposer(text) {
			...
			const mark = () => markGhost(ed);
			setTimeout(() => {
				if (ed.isConnected && ed.textContent.length > before) { mark(); return; }
				// 回退方案：逐行 execCommand
				try {
					ed.focus({ preventScroll: true });
		...
		// 给输入框里含批注标记的顶层段落打上隐藏属性
		function markGhost(ed) {
			if (!ed) return;
			for (const el of ed.querySelectorAll("p, div, li")) {
				const t = el.textContent || "";
				if (t.includes("【注")) el.setAttribute("data-stc-ghost", "");
				else if (el.hasAttribute("data-stc-ghost") && !t.includes("【注")) el.removeAttribute("data-stc-ghost");
			}
		}
		...
		function ensurePill() {
			...
			row.appendChild(ok);
			const hint = document.createElement("div"); // 说明文字已按 Codex 风格去掉
		...
		function addAnnotation(range, comment) {
			...
			anns.push(ann);
			renderBar();
			insertIntoComposer(blockText(ann));
			ensureLoop();
		}
		//#endregion
		...
		function renderBar() {
			...
			const foot = document.createElement("div"); // 已去掉
		...
		// 发送后自动清空。
		// 先等拼接落定（批注真正出现在输入框里），再守着它消失。
		// 发送后自动清除：
		// DSH 发完消息不一定清空输入框，所以不等“批注消失”，
		// 而是发送动作确认后主动把批注块移出输入框并清空清单。
