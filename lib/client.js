...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 发送后自动清空：以「输入框里的批注段消失」为准，
		// 不依赖输入框是否完全清空（DSH 发完后可能仍有残留）。
		function scheduleClearAfterSend() {
			...
			const timer = setInterval(() => {
				tries += 1;
				if (!anns.length) { clearInterval(timer); return; }
				const ed = findComposer();
				const text = ed ? (ed.textContent || "") : "";
				const hasBlock = text.includes("【注");
				if (hasBlock) { injectedSeen = true; }
				// 拼接过、且现在不在了 -> 已发出
				if (injectedSeen && !hasBlock) {
					clearInterval(timer);
					while (anns.length) dropAnnotation(anns[anns.length - 1], false);
					renderBar();
					return;
				}
				// 兜底：拼接后 6 秒内输入框变空，也当作已发出
				if (injectedSeen && !text.trim()) {
					clearInterval(timer);
					while (anns.length) dropAnnotation(anns[anns.length - 1], false);
					renderBar();
					return;
