...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function injectBeforeSend(done) {
			...
			if (text.includes("【注")) { done(); return; } // 已经在里面了，别重复
			injecting = true;
			const before = ed.textContent.length;
			const s2 = sel();
		...
		// 点发送按钮：捕获阶段先注入批注，再放行这次点击
		function onDocClickSend(e) {
			if (!anns.length || injecting) return;
			...
			injectBeforeSend(() => { try { clone(); } catch (err) { /* 忽略 */ } });
		}

		function onKeyDown(e) {
			// 回车发送：先把批注注入输入框，再重放这次回车
			if (e.key === "Enter" && !e.shiftKey && !e.isComposing && e.keyCode !== 229 && anns.length) {
				const ed = findComposer();
				if (ed && !injecting && !(ed.textContent || "").includes("【注")) {
					e.preventDefault();
					e.stopPropagation();
