...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			`.stc_pill{position:fixed;z-index:${Z};box-sizing:border-box;width:300px;`,
			...
			`.stc_input{flex:1;min-width:0;height:32px;background:var(--dsw-alias-bg-layer-1,#1d1d21);`,
				`color:var(--dsw-alias-label-primary,#ececf0);border:1px solid var(--dsw-alias-border-l1,#37373e);`,
				`border-radius:8px;padding:0 10px;font:13px/32px var(--dsw-font-family,system-ui);outline:none}`,
		...
		let layer = null;
		let chip = null, panel = null, expanded = true, clearArmed = 0;
		...
		function placeBadge(ann) {
			if (!ann.range.startContainer.isConnected) { dropAnnotation(ann, false); return; }
			const rect = ann.range.getBoundingClientRect();
			if (rect.width === 0 && rect.height === 0) { dropAnnotation(ann, false); return; }
			const el = ann.badge;
		...
		function ensurePill() {
			...
			pillInput = document.createElement("input");
			pillInput.className = "stc_input";
			pillInput.placeholder = "写下对这段的评论（可留空）";
			...
			hint.innerHTML = "<b>Enter</b> 添加 · <b>Esc</b> 取消 · 可继续划词添加多条";
			pill.appendChild(row);
			pill.appendChild(hint);
			pillInput.addEventListener("keydown", (e) => {
				if (e.key === "Enter") { e.preventDefault(); addFromPill(); }
				else if (e.key === "Escape") { e.preventDefault(); hidePill(); }
			});
			pill.addEventListener("mousedown", (e) => e.preventDefault()); // 保住页面里的选区
		...
		function hidePill() {
			pendingRange = null;
			clearHighlight();
			if (pill && pill.isConnected) { pill.style.display = "none"; }
		}
		...
		function addFromPill() {
			...
			if (editN) {
				...
				if (ann) {
					ann.comment = comment;
					if (removeBlock(ann)) insertIntoComposer(blockText(ann));
					else showToast(`未在输入框找到「注${ann.n}」原文块，评论已更新到清单`);
					renderBar();
				}
				return;
			}
		...
		function addAnnotation(range, comment) {
			...
			anns.push(ann);
			renderBar();
			if (!insertIntoComposer(blockText(ann))) showToast("未找到输入框，注释未写入（仅保留在清单）");
			ensureLoop();
		}
		//#endregion
		...
		function renderBar() {
			...
			clear.textContent = clearArmed && Date.now() < clearArmed ? "确认清空?" : "✕";
			clear.title = "清空全部注释（同时从输入框移除对应引用块）";
			clear.addEventListener("click", () => {
				if (!(clearArmed && Date.now() < clearArmed)) {
			...
			for (const ann of anns) {
				...
				del.addEventListener("click", () => {
					if (!removeBlock(ann)) showToast(`未在输入框找到「注${ann.n}」原文块（可能已被手动改动），仅清除跟踪`);
		...
		function dropAnnotation(ann, render = true) {
			...
			if (anns.length === 0) { nextN = 1; expanded = true; }
			if (render) renderBar();
		}
		//#endregion
		...
		function onMouseUp() {
			setTimeout(() => {
				if (pill && pillInput === document.activeElement) return; // 正在评论输入中
			...
			if (alive || (chip && chip.style.display !== "none")) {
				...
				if (ts - lastTick > 1200) {
					...
					for (const ann of anns.slice()) {
						const inDoc = ann.range.startContainer.isConnected;
						const inBox = text.includes(`【注${ann.n}】`);
						if (!inDoc || (!inBox && ed)) {
							dropAnnotation(ann, false);
							renderBar();
						}
