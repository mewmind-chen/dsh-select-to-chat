...
- **批注清单**：输入框上方显示「N 条注释」面板，每条含「所选文本 / 用户评论」，
  支持 ✎ 编辑评论、🗑 删除整条、✕ 一键清空（无需二次确认）；**默认收起**为
  「N 条注释」小条，**鼠标移上去即临时展开**，移开自动收起；点击可固定展开，
  再点收起，点面板外也会收起，不遮挡正文（v0.1.3）。
- **随消息发送**：输入框保持干净（不显示引用块）；在你按回车发送或点发送按钮时，
  插件自动把这批批注拼到消息末尾一起发出，**发送后自动清除**角标与清单：
...
键盘：`Enter` 添加 · `Shift+Enter` 换行 · `Esc` 关闭；长评论输入框自动增高（v0.1.2）。

清单：鼠标移到「N 条注释」上临时展开，可移入清单编辑或删除；移开约 0.2 秒后收起。
点一下该小条则固定展开，再点收起（触屏同样可用）。
...
- v0.1.4 起批注不再写入输入框：输入框里手动粘贴的文字不会被当成批注；
